...
#else
#include <linux/padconfig_santiago.h>
#endif
...
int board_init(void)
{
	...
	if ((__raw_readl(0x480029c0) & 0xff) == 0x6) { /* Booted SD slot? */
		bootdev = BOOTDEV_SDCARD;
	}
	
	gpmc_init();		/* in SRAM or SDRAM, finish GPMC */
 	gd->bd->bi_arch_number = MACH_TYPE_SANTIAGO; /* Linux mach id */
...
void board_env_init()
{
        ...
        sprintf(console, "ttyO%d", CONFIG_CONS_INDEX - 1);
        setenv("kernel.console", console);
}
